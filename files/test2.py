...
tools = [HoverTool(tooltips=[("varname", "@varname"),("varvalue","@varvalue")]), TapTool(),BoxZoomTool(),WheelZoomTool(),UndoTool(),ResetTool(),CrosshairTool()]
...
def update():
    ...
    varname_list = [str(x['var_opti_name']) for x in collconnect.find()[:update_num]]
    varvalue_list = [str(x['var_opti']) for x in collconnect.find()[:update_num]]
    ...
    source.data = dict(
        x=x_list,
        y=y_list,
        varname = varname_list,
        varvalue =varvalue_list,
    )
def update_frepoint():
    update_num = int(numline.value)
    y_list = [x['value'] for x in collconnect.find()[:update_num]]
...
def update_axis():
    ...
    p.y_range = Range1d(start=yminval, end=ymaxval)
#plot_lay = column(p,q)
line_plot_lay = layout(numline,p,controls)
